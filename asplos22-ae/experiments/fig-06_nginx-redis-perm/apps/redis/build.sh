...
# Upstream archive moved from antirez/redis to redis/redis.
# Keep the lib URL in sync so Kraft can fetch redis-5.0.6 sources.
REDIS_LIB_MK="/usr/src/unikraft/libs/redis/Makefile.uk"
if [[ -f "${REDIS_LIB_MK}" ]]; then
  sed -i \
    's#https://github.com/antirez/redis/archive/\$(LIBREDIS_VERSION).zip#https://github.com/redis/redis/archive/refs/tags/\$(LIBREDIS_VERSION).zip#g' \
    "${REDIS_LIB_MK}"
fi
