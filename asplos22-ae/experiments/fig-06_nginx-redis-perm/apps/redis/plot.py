...
def plot(permutations={}, output_file=None, metric='GET'):
  if len(permutations.keys()) == 0:
    print("No data ):")
    return

  metric = metric.upper().strip()
  if metric not in {'GET', 'SET'}:
    raise ValueError("metric must be GET or SET")
  ...
  permutations_perf = {}
  for taskid in permutations:
    if metric in permutations[taskid]:
      permutations_perf[taskid] = permutations[taskid][metric]["5"][MEAN_KEY]
    else:
  ...
  for taskid in permutations_sorted:
    ...
    _get_min = 0
    _get_max = 0
    if metric in permutation:
      _get = permutation[metric][chunk][MEAN_KEY]
      _get_min = permutation[metric][chunk][AMIN_KEY]
      _get_min = _get - _get_min
      _get_max = permutation[metric][chunk][AMAX_KEY]
      _get_max = _get_max - _get

    _set = 0
    _set_min = 0
    _set_max = 0
    if "SET" in permutation:
      _set = permutation["SET"][chunk][MEAN_KEY]
      _set_min = permutation["SET"][chunk][AMIN_KEY]
      _set_min = _set - _set_min
      _set_max = permutation["SET"][chunk][AMAX_KEY]
      _set_max = _set_max - _set

    get_matrix.append(_get)
    get_min_matrix.append(_get_min)
    get_max_matrix.append(_get_max)
    set_matrix.append(_set)
    set_min_matrix.append(_set_min)
    set_max_matrix.append(_set_max)
  ...
  nonzero_vals = [v for v in get_matrix if v > 0]
  if nonzero_vals:
    vmin = min(nonzero_vals)
    vmax = max(nonzero_vals)
    span = max(vmax - vmin, 1.0)
    chart_min = max(0, vmin - span * 0.15)
    chart_max = vmax + span * 0.20
  else:
    chart_min = 0
    chart_max = 1
  ...
  fig = plt.figure(figsize=(16.0, 4.4))
  fig.patch.set_facecolor('white')
  fig.add_artist(Rectangle((0.005, 0.005), 0.99, 0.99,
    transform=fig.transFigure, fill=False, linewidth=2.0, edgecolor='#111111'))
  ax1 = fig.add_subplot(1,1,1)
  ax1.set_facecolor('white')
  for spine in ax1.spines.values():
    spine.set_linewidth(1.8)
    spine.set_color('#111111')
  ax1.set_ylabel(r'Average Redis ' + metric + "\n" + r'request/s (x1000)',
    fontsize=SMALL_SIZE,
  ...
  ax1.tick_params(axis='x', colors='#333333', width=1.4)
  ax1.tick_params(axis='y', colors='#333333', width=1.4)
  # ax1.legend()
  ...
  ax1.bar(x, get_matrix, width,
    ...
    error_kw={'ecolor': '#2f2f2f', 'elinewidth': 0.8, 'capsize': 1.5},
    # linewidth=1,
  )
  ...
  for i, val in enumerate(get_matrix):
    ...
    if i % 4 == 0:
      text_offset = max((chart_max - chart_min) * 0.03, 1.0)
      ax1.text(i, val + text_offset, "%3.0fk" % (val/1000),
        ha='center',
  ...
  the_table.scale(1, 1.1) # table cell padding
  the_table.auto_set_font_size(False)
  the_table.set_fontsize(MEDIUM_SIZE - 1)
...
if __name__ == "__main__":
  if len(sys.argv) < 4:
    print("Usage: ./plot.sh PERMUTATIONS_FILE RESULTS_FILE OUTPUT_FILE [GET|SET]")
    sys.exit()

  metric = "GET"
  if len(sys.argv) >= 5:
    metric = sys.argv[4].upper().strip()
