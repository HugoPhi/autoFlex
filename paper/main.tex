...
\subsection{系统展示（Web自动化平台：设计与现状说明）}

为进一步降低复现与使用门槛，本文在仓库最外层实现了
website
目录下的本地 Web 自动化平台，将 AutoFlex
现有脚本化流水线封装为可交互任务系统。平台目标是让用户在不改动既有研究代码的前提下，以最少配置完成``迁移—评测—搜索—绘图''全流程执行与结果查看。当前版本重点支持本地部署、单机复现实验与产物归档。平台提供以下能力：
\begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \item
        任务提交与编排：支持 auto\_migration\_eval、search\_eval 与 unified\_plot
        三类作业；其中前两类分别复用 autoGen 与 search 子系统脚本，unified\_plot
        复用根目录统一绘图入口。
  \item
        软连接复用机制：平台通过初始化脚本将 autoGen、search、figures、generate\_figure.py
        等目录或文件软连接到 website/linked 下，避免重复维护与代码分叉。
  \item
        本地可视化控制台：提供作业状态轮询、日志尾部查看、产物列表与单文件下载能力，便于快速审阅迁移报告与搜索图表。
  \item
        结果落盘与可追溯：每个作业在 website/jobs 下保存元数据与运行日志，并把图表或评测结果写入既有产物目录（如 figures/search、autoGen/eval\_results），与论文实验数据组织方式保持一致。
\end{enumerate}

平台后端采用轻量 Web 服务实现任务编排，不重写算法本体，而是直接以子进程方式调用原有命令行脚本。该``薄封装''策略使平台具备两项工程优势：第一，可直接复用已验证的研究脚本与参数体系，降低二次实现误差；第二，算法迭代无需同步改写 Web 逻辑，只需保持脚本接口兼容。

当前实现采用三层结构：
\begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \item
        表现层（Web UI）：用于提交作业参数、查看作业状态、检索日志与下载产物。
  \item
        编排层（Job Orchestrator）：负责参数校验、命令拼装、后台执行、状态流转（queued/running/succeeded/failed）与异常记录。
  \item
        执行与存储层：直接调用 autoGen/search/generate\_figure 相关脚本并写入统一产物目录，同时为每个作业保存 metadata 与日志。
\end{enumerate}

本地部署流程较为直接：在 website
目录执行软连接初始化脚本、安装依赖并启动服务后，即可通过浏览器访问本地控制台。该流程已满足本文``系统展示''章节的演示与复现需求。

对于云端部署，本文不要求在实验中依赖云环境，但平台可按相同方式部署到云主机：在云端安装依赖后运行 Web 服务进程（如 gunicorn），再通过反向代理提供统一入口与访问控制。由于平台核心仍是对本地脚本的编排封装，云端部署与本地部署在功能语义上保持一致，差异主要体现在进程管理、存储配额与网络安全策略。
