...
传统隔离机制长期面临一个结构性矛盾：一方面，应用希望在安全上更强（更小 TCB、更强故障隔离、更少跨组件影响面）；另一方面，又希望在性能与可用性上更好（低时延、高吞吐、低迁移成本、与既有生态兼容）。更现实的问题是，许多操作系统在\emph{设计时}就将“隔离与安全原语”的组合锁定为固定策略（例如某种硬件/软件机制的混合），而在\emph{部署后}再修改这些高层安全决策往往意味着大规模重构与长期工程投入\textsuperscript{[1,2]}。与此同时，现代应用的安全/性能需求差异极大，新硬件隔离机制不断出现、旧机制也可能被证明不可靠，这使得“固定策略”的做法越来越难以覆盖真实世界的多样化约束\textsuperscript{[1,2]}。

因此，与其追求一套放之四海而皆准的隔离方案，更务实的方向是让系统支持\textbf{按应用、按组件、按部署环境}进行安全/性能策略特化：把“隔离后端、组件划分、共享策略与硬化选项”从设计时决策下放到构建/部署时决策，并为用户提供可解释、可审阅的配置与产物\textsuperscript{[1,2]}。FlexOS 正是在这一动机下提出“可组合隔离”的 LibOS 设计：将系统拆成细粒度组件，并允许不同组件选择不同隔离后端，同时用统一的 gate 抽象承载跨域调用语义\textsuperscript{[1,2]}。

作为背景对照，进程级隔离具备成熟的权限模型与故障隔离能力，但在高频交互时受制于上下文切换、页表切换与拷贝成本；容器与命名空间在资源隔离上更轻量，但共享内核使隔离不彻底；硬件辅助隔离（如 VT-x/AMD-V、MPK 等）边界更强，但传统系统往往缺少面向多组件的可组合策略接口，导致开发者很难对组件的重要性与攻击面差异进行“按需隔离”，最终经常采取“一刀切”\textsuperscript{[7,8,19]}。
...
unikernel
强调``应用与内核融合''以获得极致性能，但其单地址空间特性使内存安全与故障隔离成为核心短板\textsuperscript{[4,6,21,22]}。语言安全型系统（如以Rust为核心的单地址空间设计）能够通过类型系统降低内存破坏风险，但对大量遗留C代码的迁移成本较高。

\subsubsection{研究方向分析}

从 FlexOS 引言提出的观察出发，操作系统架构往往与其依赖的保护机制高度耦合：系统在\emph{设计时}就会对一组“高层安全决策”做出固化选择（例如硬件隔离、软件验证、运行时检查等的组合），而在\emph{部署后}再去调整这些决策通常既罕见也代价高昂\textsuperscript{[1,2]}。这种“机制锁定”解释了为何操作系统设计空间长期呈现为若干相对稳定的阵营：微内核更偏向通过 IPC 与更强的硬件边界来换取可验证性与故障隔离，但往往牺牲性能与生态兼容；单体内核在特权分离与多地址空间上形成成熟工程体系，但默认将内核代码视为可信基座；而单地址空间 OS（SASOS）一端尝试在地址空间内部引入隔离与约束，另一端则几乎放弃保护以追求极致性能，典型代表之一就是库 OS/unikernel 生态\textsuperscript{[4,6,20,21,22]}。更关键的是，许多“事后改造”并不只是替换一两个组件：例如试图移除或弱化传统特权边界、或将一个进程再细分为多个隔离域，往往涉及 ABI、内存布局、调度与 I/O 路径的系统性重构，因此工程成本极高\textsuperscript{[1,2]}。

为便于把上述“设计空间”直观化，图~\ref{fig:os-kernel-design-space-placeholder} 给出一个示意：不同内核架构在安全与性能轴上的相对位置不同，而 FlexOS 试图解锁的是一片可调的折中区域，使隔离/保护原语的组合能够在构建或部署阶段被重新选择\textsuperscript{[1,2]}。

\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\linewidth]{drawio/OS_search_area.pdf}
  \caption{配OS kernels 的安全--性能设计空间示意}
  \label{fig:os-kernel-design-space-placeholder}
\end{figure}

这种锁定在现代应用背景下会暴露出更尖锐的矛盾：一方面，按应用特化操作系统的需求持续上升，不同工作负载对性能与安全的约束差异很大；另一方面，同一应用内部也常由多个组件构成，不同组件的信任程度与关键性不同，理想的隔离强度也应当不同\textsuperscript{[1,2]}。如果过早把系统锁死在某一种安全原语组合上，往往会导致在许多场景中出现“安全过剩但性能不达标”或“性能足够但风险不可接受”的次优解\textsuperscript{[1,2]}。同时，硬件厂商与研究界持续提出新的隔离机制，旧机制也可能在实践中暴露缺陷或被迫调整威胁模型；当多种机制都可用于同一任务时，“哪一种最合适”会依赖具体硬件、应用结构、共享需求与预算约束，因而更适合被推迟到部署/构建阶段再决定\textsuperscript{[1,2]}。

从机制抽象角度看，隔离后端又可按“\emph{特权驱动}”与“\emph{世界驱动}”两类范式理解：前者（如 MPK、TEE、硬件能力等）以“当前特权集合”约束可访问数据，并提供显式域切换；后者（如 TrustZone 的 world、EPT/VM 的 guest）更接近将不同 compartment 视为不同系统，跨域交互主要通过 RPC 与共享内存完成\textsuperscript{[1,2]}。这一视角之所以重要，是因为它给出 FlexOS 兼容多后端的最小条件：只要后端（1）实现 protection domain 与域切换，且（2）支持某种形式的共享内存以承载跨域通信，就可被纳入同一可组合框架\textsuperscript{[1,2]}。

在上述脉络下，FlexOS 给出一种更“可延期决策”的折中路径：将系统拆分为细粒度组件（库），并把“组件如何划分到 compartment、每个 compartment 选择何种隔离后端、采用何种共享策略、叠加哪些软件硬化（CFI/ASan 等）”变为构建期可配置项\textsuperscript{[1,2]}。这使得隔离强度可以随组件差异而变化，而不是整机“一刀切”；也使得当硬件环境变化或隔离机制需要替换时，开发者可以通过改变配置与重新实例化来移动到新的安全/性能点位，而不必从头重构系统\textsuperscript{[1,2]}。

\subsubsection{FlexOS的解决方案}

FlexOS 的核心机制可以概括为两类可审阅的源码级原语：\textbf{call gates} 与\textbf{共享数据注解}\textsuperscript{[1,2]}。在 Unikraft 这类典型 LibOS/unikernel 中，默认情况是“同一保护域内的直接函数调用”；而在 FlexOS 源码中，跨库调用被显式写成抽象 call gate，并在构建时被工具链替换为具体后端实现\textsuperscript{[1,2]}。这种“先抽象、后实例化”的设计使 FlexOS 能同时覆盖“同域退化为普通调用”“跨域需要域切换（如 MPK）”“跨系统通过 RPC（如 VM/EPT）”等情形，并保持同一套上层代码在不同后端下可复用\textsuperscript{[1,2]}。

为了把“抽象 gate + 共享数据注解”如何落到具体代码说清楚，可参考 FlexOS 论文给出的典型三阶段变换：开发者先在源码中标注需要跨库共享的数据，并将跨库调用替换为抽象 gate；随后工具链在构建期依据配置把抽象原语实例化为具体后端实现\textsuperscript{[1,2]}。其形式可概括为：
如图~\ref{fig:flexos-porting-instantiation} 所示，FlexOS 将“跨组件交互”从隐式的直接函数调用，转化为\emph{显式、可配置、可实例化}的源码级原语\textsuperscript{[1,2]}。具体而言，移植前同一地址空间内的调用并不携带“隔离边界”的语义；移植后，开发者在调用点将跨库调用改写为抽象 gate（如 \texttt{flexos\_gate(...)}），并对可能跨库可达的数据对象施加共享注解（如 \texttt{\_\_shared}）以显式声明共享范围\textsuperscript{[1,2]}。随后工具链在构建期依据目标配置把这些抽象原语实例化为具体后端实现：当调用双方位于同一 compartment 时，gate 可退化为原始调用路径；当调用跨越 compartment 边界时，gate 则被替换为后端 gate（如 MPK/EPT/SFI 等）并插入域切换与必要的隔离维护逻辑，从而形成“移植前（直接调用）→移植后（抽象 gate + 显式共享注解）→构建期实例化（按后端/同域退化）”的三阶段变换\textsuperscript{[1,2]}。

\begin{figure}[htbp]
  \centering
  \IfFileExists{drawio/flexos_porting_instantiation.pdf}{
    \includegraphics[width=0.95\linewidth]{drawio/flexos_porting_instantiation.pdf}
  }{
  ...
  \caption{FlexOS 的三阶段变换：从直接调用到抽象 gate，再到后端实例化（占位）}
  \label{fig:flexos-porting-instantiation}
\end{figure}

这个例子同时揭示了 FlexOS 的一个关键工程前提：跨库共享并非默认允许，而是以“默认私有、按需共享”的方式显式列出共享对象与共享范围（可视为按库组/compartment 组织的共享白名单或 ACL），并由后端与工具链共同决定共享对象在不同隔离机制下应当如何落位与访问\textsuperscript{[1,2]}。在这一语义下，gate 的“调用语义”与共享数据的“可达性语义”被刻意绑定：当跨域调用发生时，调用双方必须对共享对象达成一致的可达性约束（哪些对象允许跨域、通过何种共享内存/缓冲区承载、访问路径与共享布局如何被实例化），否则即便调用点已被改写为 gate，程序也可能因指针不可达或共享布局/访问策略不一致等原因而无法正确运行\textsuperscript{[1,2]}。因此，FlexOS 将共享数据注解提升为一等公民，意在把跨域可达性从“隐式假设”变为“可审阅的显式契约”，并将其交由构建期实例化过程在不同后端下落到具体实现\textsuperscript{[1,2]}。

从更宏观的流程看，FlexOS 将“灵活隔离”拆解为一条可审阅、可复现的构建期流水线，并把关键决策推迟到构建/部署阶段\textsuperscript{[1,2]}。在源码层面，开发者（或半自动工具）将跨库交互显式化：一方面把跨库调用统一改写为抽象 gate（从语义上标出潜在的隔离边界）；另一方面通过共享注解列出跨库可达的数据对象与共享范围，从而为后续的隔离后端实例化提供必要的“共享契约”信息\textsuperscript{[1,2]}。在配置层面，开发者指定库到 compartment 的映射关系、为每个 compartment 选择隔离后端，并为共享策略与可选的软件硬化（如控制流相关的约束、运行时检查等）选择组合方式\textsuperscript{[1,2]}。这些配置共同决定了系统将落在怎样的安全/性能点位，并决定哪些跨库交互会成为跨域交互，从而需要由后端 gate 进行隔离维护\textsuperscript{[1,2]}。

在此基础上，构建系统在编译/链接阶段生成并注入后端相关的实例化产物\textsuperscript{[1,2]}。首先，工具链会依据“库—compartment—后端”的映射关系，判定每个抽象 gate 调用在该配置下是同域调用还是跨域调用：对同域调用，gate 退化为原始调用路径以尽量保留性能；对跨域调用，gate 被实例化为后端 gate，并在进入/退出边界处插入后端所需的域切换与隔离维护逻辑（例如 MPK 的权限切换与必要的状态保存/清理，或在更重的边界下采用与后端匹配的跨域调用实现）\textsuperscript{[1,2]}。其次，工具链会据共享注解与后端约束生成共享内存/缓冲区布局与访问路径，并生成必要的 glue 代码与链接期产物（如与实例化产物相匹配的符号组织与链接脚本），以保证跨域调用所需的数据可达性与运行时支撑在目标后端下成立\textsuperscript{[1,2]}。最终，FlexOS 得到的是“同一套上层源码、不同后端与划分配置”的可执行镜像：隔离边界、共享策略与硬化选项随着配置变化而变化，从而让系统能够在安全/性能折中点上随配置移动，而不必将决策固化为单一内核架构或单一隔离机制\textsuperscript{[1,2]}。

如图~\ref{fig:flexos-instantiation-workflow-placeholder} 所示，上述流程可抽象为“源码标注—配置选择—构建期实例化—运行期执行”的闭环：源码侧提供跨域交互的\emph{显式切入点}（gate）与跨域可达性的\emph{显式契约}（共享注解），配置侧提供“哪些库被分到哪些 compartment、采用哪种后端与共享/硬化策略”的\emph{可变决策}，构建侧将抽象原语与配置决策结合生成后端相关的实例化产物（包括 gate 实现、共享内存布局与 glue 代码），运行期则在真实的跨域调用路径上兑现隔离语义\textsuperscript{[1,2]}。这一分层使得 FlexOS 既能在同域场景下尽量保持接近原生的调用路径，又能在跨域场景下以统一抽象承载不同后端的隔离维护逻辑，从而将“隔离强度的选择”从设计时前移到构建期可配置项\textsuperscript{[1,2]}。
...
\begin{figure}[htbp]
  ...
  \caption{FlexOS 灵活隔离的构建期实例化流程（占位）}
...
从实现与语义层面看，FlexOS 强调 call gate 对 System V ABI 透明：caller/callee 仍按同一 ABI 传参与返回，但 gate 需要保证隔离语义，因此会更严格地处理寄存器集合（例如保存并清零不作为参数的寄存器），避免跨域泄漏或污染\textsuperscript{[1,2]}。此外，FlexOS 的 call gates 不采用 trampoline，而是\textbf{在调用点内联}替换原始函数调用：这既减少了间接跳转的额外开销，也带来一项额外的安全副收益——由于入口点在编译期被枚举与约束，库只能通过明确的 entry points 被进入，从而形成一种廉价但不完备的 CFI 约束\textsuperscript{[1,2]}。

与 call gate 配套的是共享数据注解：FlexOS 以“默认私有、按需共享”的数据所有权思路组织共享，开发者通过 \texttt{\_\_shared} 等注解为变量建立共享白名单（可视为按库组的 ACL），并由工具链在构建期为不同后端实例化合适的数据共享策略\textsuperscript{[1,2]}。值得强调的是，后端技术本身会反向约束共享方案：例如 MPK 往往要求共享数据落在共享内存区域，而 VM/EPT/TrustZone 类“世界驱动”后端通常要求每个 compartment 具有相对自洽的运行时（甚至需要在 TCB 侧做重复部署）\textsuperscript{[1,2]}。这也解释了 FlexOS 将 backends 与核心库共同纳入 TCB 的原因：后端不仅实现域切换，还需要通过 hook API 扩展调度、内存分配等核心路径（例如线程创建时切换到正确 domain）\textsuperscript{[1,2]}。

如图~\ref{fig:flexos-tcb-placeholder} 所示，FlexOS 的 TCB 边界通常不仅覆盖隔离后端本身（负责建立 protection domain 并执行域切换），还覆盖为 gate 调用与共享策略提供支撑的核心库/运行时扩展与相关 glue 代码\textsuperscript{[1,2]}。这是因为在 FlexOS 的模型中，后端并非仅在“切换那一刻”生效：它需要与运行时协同，确保线程/中断/内存等核心路径在跨域语义下仍保持一致（例如在进入正确域后再执行目标库代码、在共享区域中放置允许跨域访问的数据对象、以及在边界处维护必要的状态不泄漏）\textsuperscript{[1,2]}。因此，TCB 的大小与构成会随所选后端与共享策略而变化：越重的隔离边界往往需要越多的运行时支撑，而越激进的共享与优化也越依赖正确的 glue 代码把抽象契约落到具体实现；这正是 FlexOS 将“后端实现 + 核心库扩展”一并纳入可信基并强调构建期可审阅产物管理的重要原因\textsuperscript{[1,2]}。

\begin{figure}[htbp]
  \centering
  \IfFileExists{drawio/flexos_tcb.pdf}{
    \includegraphics[width=0.9\linewidth]{drawio/flexos_tcb.pdf}
  }{
    \fbox{\parbox{0.95\linewidth}{\centering
  ...
  \caption{FlexOS 的 TCB 构成（占位）}
  \label{fig:flexos-tcb-placeholder}
\end{figure}

\subsubsection{缺陷与挑战}

上述抽象带来的是“可组合”，付出的代价则集中体现在\textbf{迁移工作量}与\textbf{配置空间爆炸}两方面。迁移上，FlexOS 论文指出共享数据识别往往占据 porting 的绝大部分工作量：手工注解更精确但费时，静态分析更自动但保守，混合分析在工程上更常见\textsuperscript{[1,2]}。对调用点改写而言，工具链可通过静态分析判断过程调用是否跨库边界并做语法替换，但一类典型角落案例是函数指针：callee 无法静态确定时需要开发者补充可能目标集合，随后工具链生成 wrapper 并将目标实现包裹在合适的 gate 中\textsuperscript{[1,2]}。此外，FlexOS 采用源码到源码变换（例如借助 Coccinelle）而非链接期/运行时重定向，使得变换结果可用常规 diff 直接审阅，并能继续享受编译器优化；但与此同时，构建期工具链会额外生成较多辅助代码与脚本（包括 gate 实例化、共享数据代码、链接脚本与核心库扩展等），其工程复杂度需要通过流水线化与可复现 artifact 管理来对冲\textsuperscript{[1,2]}。

配置上，FlexOS 将“隔离策略”转化为构建期多维决策：compartment 划分与库映射、隔离后端、共享策略、硬化选项等维度相乘，自然形成组合爆炸\textsuperscript{[1,2]}。这使得“最安全配置”不再是单点选择，而是一个在性能预算约束下的探索/搜索问题；也正因如此，FlexOS 与后续工作强调需要探索技术在安全/性能前沿上导航，避免人工在指数级空间中试错\textsuperscript{[1,2]}。

更进一步，从工具链角度看，这种“构建期实例化”并非仅做少量宏替换。FlexOS 的 build-time source transformations 会系统性地产生/修改多类产物：实例化抽象 gates、实例化共享数据代码、生成链接脚本、并按后端配方为核心库生成额外代码\textsuperscript{[1,2]}。论文报告在一个相对简单的 Redis 配置中，工具链修改量可达约 $\sim 1$ KLoC\textsuperscript{[1,2]}，这从侧面说明自动化与可复现流水线对于控制工程复杂度是必要条件。

不同后端还会对配置可行域施加不同约束：例如 MPK 的“键数量/权限切换模型”、EPT/VM 的“跨系统 RPC 与更重的运行时边界”、以及 SFI 的“插桩粒度与额外检查开销”等\textsuperscript{[7--9]}。这些约束不仅影响单个 gate 的实现，还会影响共享数据的组织方式、跨域缓冲区的布局、以及最终可达到的安全/性能前沿形态，从而进一步放大了“人工穷举配置”的不可行性。
